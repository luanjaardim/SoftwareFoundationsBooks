...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros. unfold valid_hoare_triple. intros. apply H.
Qed.
...
Proof.
  ...
  unfold not in H. apply H in H1. destruct H1.
Qed.
(** [] *)
...
(* Theorem hoare_asgn_wrong : exists a:aexp,
  ~ {{ True }} X := a {{ X = a }}.
Proof.
  exists <{ X + 1}>. unfold not, valid_hoare_triple. intros. simpl in H.
  (* st' X cannot be equal to st' (X + 1)! *)
  Admitted. *)
  
Theorem hoare_asgn_wrong :
 ~ (forall X a, {{True}} X := a {{X = a}}).
Proof.
  unfold not, valid_hoare_triple. intros.
  simpl in H. specialize (H X (APlus (AId X) (ANum 1)) 
    (X !-> 0) 
    (X !-> aeval (X !-> 0) (APlus (AId X) (ANum 1)); (X !-> 0))).
  simpl in H.
  assert(HAsgn: (X !-> 0) =[ X := X + 1 ]=> (X !-> 1; X !-> 0)). {
