...
Theorem mul_0_r : forall n:nat,
  n * 0 = 0.
Proof.
  intros n. induction n as [| n_ant IH].
  - simpl. reflexivity.
...
Proof.
  ...
  (* Perguntar de symmetry pode ser utilizada. *)
  (*  - simpl. rewrite <- IHm. symmetry. apply plus_n_Sm.*)
Qed.
...
Proof.
  ...
  assert (H : n + (m + p) = m + (n + p)). {
    rewrite add_assoc. rewrite (add_comm n m). rewrite <- add_assoc.
    reflexivity.
  }
...
Proof.
  ...
  - simpl. apply mult_n_O. (* mult_n_0: 0 = n * 0 *)
  - simpl. assert (H: n + m * n = n * m + n).
    { rewrite IHm. rewrite add_comm. reflexivity. }
    rewrite H. apply mult_n_Sm. (* mult_n_Sm: n * m + n = n * S m *)
Qed.
(** [] *)
...
Proof.
  intros b. 
  (* Prove commutability *)
  assert (H: andb b false = andb false b).
...
Proof.
  ...
  - simpl. rewrite IHp. do 2 (rewrite add_assoc). rewrite (add_comm n m).
     ...
     rewrite tmp. rewrite (add_comm m). reflexivity.
Qed.
